...
public class CustomSortString {
    public static void main(String[] args) {
        ...
        String sortedStr = customSortString(order, str);
    ...
    private static String customSortString(String order, String str) {
        ...
        Map<Character, Integer> aplhabet = new HashMap<>();
        int ind = 0;
        ...
        for(int i=0; i<source.length; i++) {
            if(aplhabet.containsKey(source[i])) {
                ind = aplhabet.get(source[i]);
                if(sorted[ind] == ' ') {
                    sorted[ind] = source[i];
                } else {
                    while(sorted[ind] != ' ') ind++;
                    sorted[ind] = source[i];
                }
            } else {
